...
PClip AudioDub::FinalSimplify() const
{
  ...
  if ( std::shared_ptr<KillAudio const> ka = std::dynamic_pointer_cast<KillAudio const>(GetVideoChild()) )
    SetLeftChild( ka->GetChild() );

  //absorbing KillVideo audio child
  if ( std::shared_ptr<KillVideo const> kv = std::dynamic_pointer_cast<KillVideo const>(GetAudioChild()) )
