...
CPVideoFrame VideoSwitch::GetFrame(int n, Splice const& splice) const
{
  ...
  if ( it == switchs_.end() || n < 0 )   //either after last clip or before first one
    throw exception::NoSuchFrame(n);     //report frame don't exist

  if ( it != switchs_.begin() )          //if not at begin
    n -= it[-1];                         //we must substract frame count of the previous clips

  //and now we get the corresponding child from the splice to get the frame
  return splice.GetChild( it - switchs_.begin() )->GetFrame(n);
}
