...
void Resize::Horizontal::YUY2::ResizeFrame(CPVideoFrame source, PVideoFrame target) const
{
  ...
  BYTE * tempY = GetEnvironment()->TempBuffer( src.width * 8 * 2 + 16, 8 );
  BYTE * tempUV = tempY + src.width * 8 + 8;

  int const * pptrY  = luma.get();
  int const * pptrUV = chroma.get();

  int countY  = luma.count() >> 2;        //we use 4 coeffs per  Y loop
  int countUV = chroma.count() >> 1;      //we use 2 coeffs per UV loop 
  ...
  for( int y = dst.height; y-- > 0; src.to(0, 1), dst.to(0, 1) )
  {
    ...
    xloop:
      ...
      movd        mm1, mm0                //mm1 = 0000000000001FFF    init rounded
      movd        mm2, mm0                //mm2 = 0000000000001FFF    init rounded
      movd        mm3, mm0                //mm3 = 0000000000001FFF    init rounded
