...
CPVideoFrame RawVideo::MakeFrame(long n) const
{
  GetVideoInfo()->CheckHasFrame(n);                  //defends against illegal values

  long startIndex = PreviousKeyFrame(n);             //find the last keyframe
  ...
  {
    CPVideoFrame frame = frameDecompressor_->operator()(i, i != n);
